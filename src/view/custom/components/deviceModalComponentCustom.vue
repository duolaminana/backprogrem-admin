<template>
  <Card class='typeTemplateDetails'>
    <div class=headBox>
      ...
      <strong style="font-size:16px">{{formValidate.machineName||query.machineName}}</strong>
    </div>
    <Divider />
    <div style="overflow: hidden;">
      <Button type="error" size="large" @click='deleteRow' style="float:right;">删除行</Button>
      <Button type="primary" size="large" @click='addRow'  style="margin-right:10px;float:right;">添加行</Button>
    </div>
    <Card class='box' :padding='6'>
      ...
      <div class='rowBefore'>
        ...
        <Button type="primary" icon="md-add" @click='addClomun(i)'></Button>
      </div>
...
export default {
  ...
  props:['machineCode','formValidate','query'],
  filters:{
    prefixValue(value){
  ...
  methods:{
    addClomun(i){
      ...
      let data = {};
      data.id = null;
      // data.roadNo = previous.roadNo+1||(i+1-1)*1;
      data.roadNo = null;
    ...
    async keep(){
      if(this.listData.length){
        ...
        if(!this.formValidate.isAutomatic){
          this.$Spin.show()
          let ary = [];
          ...
          let data = {
            list:ary,
            operatorName:null,
            channelId:this.formValidate.channelId
          }
          let d = {
            addMachineTypeDto:this.formValidate,
            addMachineTypeRoadListDto:data,
          }
          console.log(data)
          console.log(this.formValidate)
          await netWorkDevice("/machineType/addCustom", d).then(res => {
            this.$Message.success("操作成功");
            this.$emit('toBack');
            this.$Spin.hide()
          }).catch(()=>{
