<template>
  <div>
    ...
    <Modal
      v-model="newlyAdded"
      width="600"
      :title="showNewlyType=='xz'?'新增路线':'编辑路线'"
      :mask-closable="false"
    >
      <h4
        style="padding:0 0 10px 60px"
        v-if="pickTreeData&&pickTreeData.name"
      >已选{{pickTreeData.routeType==1?'分区':'线路'}}：{{pickTreeData.name}}</h4>
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        remark: [{ max: 20, message: "长度最多是20个字符", trigger: "blur" }]
      },
      newlyAdded: false,
      showNewlyType: "xz",
      routeId: "",
  ...
  methods: {
    ...
    channerChanges(value, selectedData) {
      this.channelId =
        value[value.length - 1] || this.$store.state.user.channelId;
      this.getTreeData();
    },
    del() {
      this.modal_loading = true;
      let url = `/route/deleteRoute?id=${this.delID}&&routeType=${this.delRouteType}`;
      netWorkDevice(url, null, "delete")
        .then(res => {
          this.modal_loading = false;
          this.modalDel = false;
      ...
      this.$refs[names].validate(valid => {
        if (valid) {
          ...
          if (this.showNewlyType == "xz") {
            ...
            let data = {
              routeName:
                routeType == 1 ? routeName + "(分区)" : routeName + "(线路)",
              remark,
              routeType,
              pid: this.pickTreeData ? this.pickTreeData.id : -1,
              pids:
                (this.pickTreeData
                  ? this.pickTreeData.parentIds
                    ? this.pickTreeData.parentIds + this.pickTreeData.id
                    : this.pickTreeData.id
                  : "") + ",",
